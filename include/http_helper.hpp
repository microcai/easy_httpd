...
namespace http
{
	...
	inline bool headers_equal(const std::string& a, const std::string& b)
	{
		if (a.length() != b.length())
			return false;
		return std::equal(a.begin(), a.end(), b.begin(), string_util::tolower_compare);
	}
	...
	private:
		void parse_multipart(const std::string& formdata, const std::string& boundary)
		{
			...
			for (auto p : parts)
			{
				...
				{
					std::string key = w[1];
					string_util::unescape_path(key, h);
			...
			for ( auto C : formdata)
			{
				...
				else
				{
					...
					else
					{
						...
						string_util::unescape_path(key, k);
						string_util::unescape_path(value, v);
